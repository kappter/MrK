/*************************************************************************
*  24bit RGB Color Visualization Tool: By Ken Kapptie | Copyright 2013   *
*  Goal: to teach OOP to high school students while creating something   *
*        useful and highly customizable that runs on most OS's.          *
*  TODO: 1. Employ a more attractive UI                                  *
*        2. Type custom values                                           *
*        3. Copy and paste hex values for other apps                     *
*                                                                        *
**************************************************************************/
...
void draw() {
  ...
  text("1 Byte Red",508,53);
  fill(redVal,0,0);
  rect(500,60,100,100,8);
  ...
  text("1 Byte Green",498,243);
  fill(0,greenVal,0);
  rect(500,250,100,100,8);
  ...
  fill(200);
  textSize(30);
  text("1 pixel of 24bit color...", 133,720);
}
  ...
  if (mouseY > 60 && mouseY < 160) {
    for (int i = 0; i < redSwitches.length; i++) {
      if (mouseX > 20+i*60 && mouseX < (i+1)*60) {
        ...
        if (redSwitches[i].on) {
          redDecVal += (int)pow(2, redSwitches[i].binValue);
        }else {
          redDecVal -= (int)pow(2, redSwitches[i].binValue);
        }
      }
    ...
    for (int i = 0; i < greenSwitches.length; i++) {
      if (mouseX > 20+i*60 && mouseX < (i+1)*60) {
        ...
        if (greenSwitches[i].on) {
          greenDecVal += (int)pow(2, greenSwitches[i].binValue);
        }else {
          greenDecVal -= (int)pow(2, greenSwitches[i].binValue);
        }
      }
    ...
    for (int i = 0; i < blueSwitches.length; i++) {
      if (mouseX > 20+i*60 && mouseX < (i+1)*60) {
        ...
        if (blueSwitches[i].on) {
          blueDecVal += (int)pow(2, blueSwitches[i].binValue);
        }else {
          blueDecVal -= (int)pow(2, blueSwitches[i].binValue);
        }
      }
